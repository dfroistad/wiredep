...
var replaceIncludes = function (file, fileType, returnType) {
  ...
  return function (match, startBlock, spacing, blockType, oldScripts, endBlock, offset, string) {
    ...
    dependencies.
      forEach(function (origFilePath) {
        var filePath = path.join(
            path.relative(path.dirname(file), path.dirname(origFilePath)),
            path.basename(origFilePath)
          ).replace(/\\/g, '/').replace(ignorePath, '');

        if (filesCaught.indexOf(filePath) === -1) {
          newFileContents += spacing + fileType.replace[blockType].
            replace('{{filePath}}', filePath).
            replace('{{fileContents}}', function() {
              return String(fs.readFileSync(origFilePath)).trim();
            }).
            replace(/\r\n|\r|\n/g, spacing);
        }
      });
